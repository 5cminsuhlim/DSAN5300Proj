...
#distribution of race classes played in percentage as dataframe
race1_df <- as.data.frame(table(match_hist$player_1_race))
race2_df <- as.data.frame(table(match_hist$player_2_race))
#rename cols
names(race1_df) <- c("Race", "Count")
names(race2_df) <- c("Race", "Count")
...
ggplot(race1_df, aes(x=Race, y=Count))+
  ...
  )+ labs(title = 'Distribution of Race Classes for Player 1')

ggplot(race2_df, aes(x=Race, y=Count))+
  geom_bar(stat = 'identity', fill='lightblue', color = 'black')+
  theme_minimal() +
  theme(
    panel.grid.major = element_blank(),
    ...
    axis.ticks = element_line(color = "black"),
    axis.line = element_line(color = "black")
  )+ labs(title = 'Distribution of Race Classes for Player 2')
...
ggplot(race_wr_df, aes(x = Race, y = Win_Percentage)) +
  ...
  theme(
    ...
    axis.ticks = element_line(color = "black"),
    axis.line = element_line(color = "black")
  )+ labs(title = 'Win percentage for Each Race Class') +
...
ggplot(perc_combos_filtered, aes(x = Class_Combo, y = Win_Percentage)) + geom_bar(stat='identity', fill = 'lightblue', color = 'black')+
  coord_cartesian(ylim = c(0, 100)) +
  theme_minimal() +
  theme(
    panel.grid.major = element_blank(),
  ...
  labs(x = 'Each Race vs Race Combination',
       y = 'Win Percentage (%)',
       title = 'Win Rate of Each Race vs Race Combination (1st Perspective)')
...
ggplot(iem_players) +
  ...
  geom_boxplot() +
  theme(legend.position = "none") + labs(title='SPM per Race Class (Protoss, Terran, Zerg)') + 
  scale_fill_manual(values = c('#F8DC52', '#315BE6', '#25033D'))+
  theme_minimal() +
  theme(
    panel.grid.major = element_blank(),
...
ggplot(data.frame(Residuals = apm_anova$residuals), aes(x = Residuals)) +
  ...
  labs(title = 'APM ANOVA Testing; APM ~ Race')
ggplot(data.frame(Residuals = spm_anova$residuals), aes(x = Residuals)) +
  geom_histogram(fill = "lightblue", color = "black", bins = 20)+
  theme_minimal() +
  theme(
    panel.grid.major = element_blank(),
  ...
  labs(title = 'SPM ANOVA Testing; SPM ~ Race')
